...
- many GUI fixes (e.g. `ESC` closes all dialogs, source code tab displays file name, dialog modality issues, added icon
  to windows)
...
- fix: On computer reset, 88-SIO buffer is not cleared; on automatic emulation (non-interactive), reset should not clear
  the buffer
