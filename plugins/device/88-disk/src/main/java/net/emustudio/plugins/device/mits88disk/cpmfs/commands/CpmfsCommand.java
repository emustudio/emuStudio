...
import net.emustudio.plugins.device.mits88disk.Command;
...
public class CpmfsCommand implements Command {
    ...
    @Argument(handler = SubCommandHandler.class, metaVar = "CPMFS command (cat, list, info)", required = true)
    @SubCommands({
        @SubCommand(name = "cat", impl = CatSubCommand.class),
        @SubCommand(name = "list", impl = ListSubCommand.class),
        @SubCommand(name = "info", impl = InfoSubCommand.class)
    })
