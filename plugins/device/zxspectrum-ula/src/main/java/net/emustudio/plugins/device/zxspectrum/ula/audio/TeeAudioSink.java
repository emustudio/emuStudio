 ...
 *
 * <p>{@link #flushAudio()} intentionally targets only the primary sink. The beeper uses flush during
...
final class TeeAudioSink implements AudioSink {
    ...
    public void accept(byte[] pcmSamples, int length) {
        primarySink.accept(pcmSamples, length);
        secondarySink.accept(pcmSamples, length);
    }
