...
import static net.emustudio.plugins.compiler.asZ80.CompileError.ambiguousExpression;
...
public class EvaluateExprVisitor extends NodeVisitor {
    ...
    public void visit(ExprString node) {
        ...
        if (sizeBytes == 2) {
            ...
            if (maxValue != null) {
                maxValue = Math.min(maxValue, 0xFF);
                if (strLen > 1) {
                    env.error(expressionIsBiggerThanExpected(node, 1, strLen));
                }
            } else {
                maxValue = 0xFF;
            }
    ...
    private void evalExpr(Node node) {
        ...
        if (latestEval != null) {
            ...
            latestEval = (Evaluated) latestEval.copy();
            Integer maxVal = node.getMaxValue();
            if (maxVal != null) {
                latestEval.setMaxValue(maxVal);
            }
