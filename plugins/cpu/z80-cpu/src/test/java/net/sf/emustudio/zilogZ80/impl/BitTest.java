...
public class BitTest extends InstructionsTest {
    ...
    public void testSET_b__mHL() {
        ...
        Generator.forSome16bitUnary(2,
                test.verifyByte(context -> context.first, context -> 1).run(0xCB, 0xC6),
                test.verifyByte(context -> context.first, context -> 2).run(0xCB, 0xCE),
                test.verifyByte(context -> context.first, context -> 4).run(0xCB, 0xD6),
                test.verifyByte(context -> context.first, context -> 8).run(0xCB, 0xDE),
                test.verifyByte(context -> context.first, context -> 16).run(0xCB, 0xE6),
                test.verifyByte(context -> context.first, context -> 32).run(0xCB, 0xEE),
                test.verifyByte(context -> context.first, context -> 64).run(0xCB, 0xF6),
                test.verifyByte(context -> context.first, context -> 128).run(0xCB, 0xFE)
        );
    }
