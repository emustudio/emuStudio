...
public class CpuVerifier {
    private final CpuImpl cpu;
    private final MemoryStub memoryStub;

    public CpuVerifier(CpuImpl cpu, MemoryStub memoryStub) {
    ...
    public void checkSP(int SP) {
        assertEquals(
                String.format("Expected SP=%04x, but was %04x", SP, cpu.getEngine().SP),
                SP, cpu.getEngine().SP
        );
    }
