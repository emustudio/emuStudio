...
public class debugTableModel extends AbstractTableModel {
    ...
    //    if (pc > firstRowAddress+10) 
  //          firstRowAddress = cpu.cpuGetNextPosition(firstRowAddress);
//        else if (pc < firstRowAddress) {
            firstRowAddress = 0;
            while (pc > firstRowAddress+10)
                firstRowAddress = cpu.getNextPC(firstRowAddress);
      //  }
    ...
    public int getRowAddress(int rowIndex) {
        if (rowIndex == 0) return this.getFirstRowAddress();
            ...
            for (i = 0; i < rowIndex; i++) a = cpu.getNextPC(a);
            return a;
        }
    }
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        if (rowIndex == 0) { nextAddress = this.getFirstRowAddress(); lastRow = 0; }
            ...
            int i = 0;
            nextAddress = this.getFirstRowAddress();
            for (i = 0; i < rowIndex; i++) nextAddress = cpu.getNextPC(nextAddress);
            lastRow = i;
        }
        addr = nextAddress;
        nextAddress = cpu.getNextPC(nextAddress);
        lastRow = rowIndex;
