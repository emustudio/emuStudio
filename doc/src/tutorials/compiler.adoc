...
Before reading on, please read the "Introduction for writing virtual computers" document. It provides the information
...
Assembler will support all forms of instructions shown in the table in section <<SSEM_ASM>>. All instructions must
...
Raw number constants can be defined in separate lines (with `NUM xxx`). The numbers themselves can be defined in decimal
or hexadecimal form. Hexadecimal format must start with prefix `0x`. For example:

    0 NUM 0x20
    1 NUM 1207943145

Only integral constants are supported, and the allowed range is from 0 - 31 (maximum is 2^5).

Word `NUM` belongs to "reserved words", but number constants to the category called "literals".
...
NOTE: Note the naming of the plug-in. We are following the naming convention as described in the
...
----
include::../../../plugins/compilers/as-ssem/src/main/jflex/lexer.jflex[]
----
...
Tokens are assigned into categories, as was already mentioned in section <<TOKEN_CATEGORIES>>. Token categories have
...
----
include::../../../plugins/compilers/as-ssem/src/main/java/net/sf/emustudio/ssem/assembler/tree/Instruction.java[]
----
...
themselves. These are called from the parser - check the grammar specfile in the section <<GRAMMAR>>.
...
[source,java]
----
include::../../../plugins/compilers/as-ssem/src/main/java/net/sf/emustudio/ssem/assembler/tree/Constant.java[]
----

Comparing `Constant` instances is based on comparing the numbers they represent.

== How to test lexer and parser
...
=== A Note to parser test

The tests are based on comparing `Instruction` and `Constant` instances with JUnit's `assertEquals()`
method. This is possible only because of overriden `equals()` and `hashCode()` methods in the classes, since they are used
...
You might remember the section <<AST>>, which talked about AST traversal. We already have `ASTnode` and `ASTvisitor`
...
as one `Integer`, which has also 32 bits. The instruction format is explained in section <<SSEM_ASM>>. First 5 bits from
...
----
include::../../../plugins/compilers/as-ssem/src/main/java/net/sf/emustudio/ssem/assembler/SeekableOutputStream.java[]
----
...
----
include::../../../plugins/compilers/as-ssem/src/main/java/net/sf/emustudio/ssem/assembler/MemoryAndFileOutput.java[]
----
