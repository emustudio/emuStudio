...
emuStudio, emuLib and some common practices. It will not dig into the compiler theory much, just what will be required
on the way.

In present days, much tedious and error-prone "mechanical" work of parsing can be generated automatically with
parser generators. All emuStudio compilers use these tools; this tutorial won't be an exception. Here they are:
...
the emulated CPU. It does not have to be assembler, it really can be any language compiler. The output of
...
of Java interfaces and some useful classes in emuLib, in package `emulib.plugins.compiler`. This tutorial will guide
you with implementing the API.
...
It it very simple computer, which can run 6 instructions.
...
For example, simple `5+3` addition can be implemented as follows:
    ...
    0 LDN 5 // load negative X into the accumulator
    1 SUB 3 // subtract Y from the value in the accumulator
    2 STO S // store the result at address S
    3 LDN S // load negative value at address S into the accumulator
...
Literals::
 - `number = (\-)?[0-9]+` or `(\-)?[0-9a-fA-F]` (hexadecimal variant), range: `<0, 31)` (max. 2^5)
Comment::
 - `comment = //[^\r\n]*`
Separator::
 - `eol = \r|\n|\r\n`
...
[source,bison]
----
Program     : number Statement Program
  | number eol Program
  | eol Program
  | &Epsilon
  ;
...
split actially an interface (because of easier testing):
...
We require the "seeking" because each instruction line begins with the "line" itself, which actually represents the
address in memory, when multiplied by 4 bytes per row.
